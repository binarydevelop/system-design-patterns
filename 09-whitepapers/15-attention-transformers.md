...
- **Prefill vs decode asymmetry.** Processing the prompt is one big parallel matmul pass (compute-bound); generating runs the *whole stack once per output token* (memory-bandwidth-bound, sequential again — autoregression reintroduced the serial chain, but only at inference). This is the two-regime split that [disaggregated serving](../17-llm-systems/05-llm-infrastructure.md) exists to exploit.
- **The KV cache is the paper's data structure made operational.** Causal attention lets each generated token reuse all previous keys/values — caching them avoids quadratic re-computation but costs `layers × heads × d × 2 × seq_len` per sequence: the memory object that PagedAttention virtualizes, prefix caching shares, MQA/GQA shrink (fewer K/V heads), and [context-management](../17-llm-systems/08-context-management.md) budgets exist to contain.
...
- Even the **agentic stack** inherits its shape: tokens-in/tokens-out autoregression is why [harness engineering](../17-llm-systems/09-harness-engineering.md) obsesses over context budgets, append-only prompts, and cache-friendly prefixes.
...
- **It created the workload class this book's [LLM Systems](../17-llm-systems/01-agent-fundamentals.md) section covers** — the first new first-class datacenter workload since web serving and MapReduce-style analytics, with its own storage hierarchy (HBM/KV/prefix caches), schedulers (continuous batching), and failure modes.
...
- [LLM Infrastructure](../17-llm-systems/05-llm-infrastructure.md) — where this paper's cost terms become your pager
