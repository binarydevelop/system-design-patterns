...
graph TD
    ...
    subgraph Txn["Transaction Layer"]
        ...
        PC[Parallel Commit Protocol<br/>2PC without blocking]
    end

    subgraph Dist["Distribution Layer"]
        RA[Range Addressing]
        Raft[Raft Consensus<br/>per range]
    end

    subgraph Store["Storage Layer"]
        Pebble[("Pebble — LSM-tree storage<br/>(formerly RocksDB)")]
    end
...
sequenceDiagram
    ...
    A-->>Co: Vote Yes
    B-->>Co: Vote Yes
    Note over Co: Commit Point
    Co->>A: Commit
    Co->>B: Commit
    A-->>Co: Ack
    B-->>Co: Ack
    Note over Co: Client waits for all 4 round trips
```
...
sequenceDiagram
    ...
    RB-->>G: Ack
    RA-->>G: Ack
    Note over G: Client gets response after 1 round trip
    Note over RA,RB: Async commit happens in background
```
...
graph LR
    subgraph NA["Node A"]
        RA["Range 42 (Leader)<br/>Leaseholder — serves reads<br/>Raft Log: 1,2,3,4,5"]
    end
    subgraph NB["Node B"]
...
> Each range is an independent Raft group. Thousands of ranges = thousands of Raft groups.
...
```mermaid
graph TD
    GW1["Gateway Node<br/>1. Parse SQL<br/>2. Plan query<br/>3. Optimize (cost-based)<br/>4. Distribute to relevant ranges"]

    N2["Node 2<br/>TableReader + Filter<br/>(local scan)"]
    N3["Node 3<br/>TableReader + Filter<br/>(local scan)"]
    N4["Node 4<br/>TableReader + Filter<br/>(local scan)"]

    GW2["Gateway Node<br/>Merge sorted results<br/>Apply LIMIT<br/>Return to client"]
