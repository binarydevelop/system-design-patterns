...
- **ジャストインタイムの検索がプリロードに勝ちます。** 関連しそうなものすべてをプロンプトに詰めるのではなく、検索ツール(`grep`、セマンティック検索)を与えて必要なものを引かせます。[エージェントコンテキストエンジニアリング](../19-compound-engineering/03-agent-context-engineering.md)参照。
