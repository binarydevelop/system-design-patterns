...
> この記事は英語版から翻訳されました。最新版は[英語版](/07-real-time/05-webrtc.md)をご覧ください。

## TL;DR

WebRTC（Web Real-Time Communication）は、サーバーリレーを必要とせず、ブラウザ間で直接的にピアツーピアの音声、映像、データ共有を可能にします。NATトラバーサルにはSTUN/TURNサーバーを使用し、接続メタデータの交換にはシグナリングサーバーを使用します。WebRTCはビデオ会議、音声通話、画面共有、リアルタイムゲーミングに最適です。
...
1. **デフォルトはピアツーピア** — WebRTCは直接接続を確立します。TURNサーバーはP2Pが失敗した場合のみ使用します

2. **シグナリングは別途** — WebRTCはシグナリングを定義しません。オファー/アンサー交換にはWebSocket、HTTP、またはその他のチャネルを使用します

3. **ICEがNATを処理** — STUNがパブリックアドレスを発見し、直接接続が不可能な場合はTURNがリレーします

4. **スケーリングにはSFU** — メッシュトポロジーはスケールしません。多数の参加者によるビデオ会議にはSFUを使用します

5. **低レイテンシにはデータチャネル** — ゲームの状態、ファイル転送、またはあらゆるリアルタイムデータにRTCDataChannelを使用します

6. **ネットワーク変更に対応** — ネットワーク切り替え（WiFiからセルラー）にはICEリスタートを実装します

7. **接続品質を監視** — getStats() APIを使用してビットレート、パケットロスを監視し、品質を調整します
