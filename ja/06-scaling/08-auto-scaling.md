...
> この記事は英語版から翻訳されました。最新版は[英語版](/06-scaling/08-auto-scaling.md)をご覧ください。

## TL;DR

オートスケーリングは、需要に基づいてコンピュートキャパシティを自動的に調整し、トラフィックスパイク時にリソースを追加し、閑散時に削減します。これによりコストを最適化しながらパフォーマンスを維持します。主要なコンポーネントには、メトリクス収集、スケーリングポリシー、クールダウン期間、ヘルスチェックがあります。最新のシステムでは、より良い応答性のために、リアクティブスケーリングと並行して予測スケーリングを使用します。
...
1. **複数のメトリクスを使用する** — CPU、メモリ、アプリケーションメトリクス（キュー深度、レイテンシ）を組み合わせて正確なスケーリングを実現します

2. **ターゲットトラッキングを優先する** — ほとんどのユースケースでステップスケーリングよりシンプルで効果的です

3. **クールダウンを実装する** — 適切なクールダウン期間でスラッシングを防止します（スケールアウトは速く、スケールインは遅く）

4. **スケジュールスケーリングを追加する** — 既知のトラフィックパターン（営業時間、イベント）に対して事前にスケールします

5. **予測スケーリングを検討する** — MLを使用してトラフィックスパイクが発生する前に予測します

6. **ヘルスチェックが重要** — 異常なインスタンスの迅速な検出と置換がキャパシティ損失を防ぎます

7. **コストを最適化する** — ワークロードの予測可能性に基づいてリザーブド、スポット、オンデマンドインスタンスを組み合わせます
