...
|---|---|
| `HIT` | キャッシュから提供 |
| `MISS` | キャッシュになし、オリジンから取得 |
| `STALE` | 期限切れエントリを再検証中に提供 |
| `BYPASS` | キャッシュをスキップ（例: `Cache-Control: no-cache`） |
| `REVALIDATED` | 条件付きリクエストでオリジンとの鮮度を確認済み |
...
HTTP/3は**QUIC** — TLS 1.3を組み込んだUDPベースのトランスポート — の上で動き、CDNエッジこそそれが報われる場所です。エッジは、損失とレイテンシが住むラストワンマイルを終端するからです。CDNはユーザーとはHTTP/3で、オリジンとは通常のHTTP/2/1.1で話します。有効化はたいていチェックボックス+`Alt-Svc`広告だけで、オリジンの変更は不要です。

| TCP+TLSの問題 | QUICの答え | エッジでの効果 |
|---|---|---|
| TCPハンドシェイク後にTLSハンドシェイク(2〜3 RTT) | トランスポート+暗号の統合ハンドシェイク: 1 RTT、再開時は**0-RTT** | ファーストバイト高速化。高RTTのモバイルで最大 |
| ヘッドオブラインブロッキング: 1パケットの喪失が多重化された全H2ストリームを止める | 独立したストリーム — あるストリームの喪失が他を止めない | 損失の多い回線でのテールレイテンシ急騰が減る |
| 接続 = 4タプル。ネットワーク切替で死ぬ | **コネクションID** — Wi-Fi↔モバイルの移動を生き延びる | 再ハンドシェイクなしでモバイルセッション継続 |
| カーネルTCPの硬直した進化 | ユーザー空間トランスポート、暗号化ヘッダ | プロトコル改善が速い(CDNはフリート全体に展開) |

有効化の前に知っておくべき運用ノート:

- **フォールバックは優雅でなければならない:** 一部のネットワークはUDP 443を遮断/絞ります。クライアントはレースするかAlt-Svc経由でH2へフォールバックします — 可用性だけでなくH3ネゴシエーション率を監視すること。
- **0-RTTデータはリプレイ可能** — 冪等なリクエストにのみ受け入れます([冪等性](../01-foundations/08-idempotency.md))。CDNはデフォルトでこれを強制しますが、オリジン経路への0-RTTを有効化する前に確認を。
- **CPUコストはカーネルTCPより高い**(ユーザー空間スタック、パケット単位の暗号) — テナント間で償却できるエッジに置くべきもうひとつの理由です。オリジンには置かないこと。
- 勝てる場所で測ること: ハンドシェイク時間と*損失の多い/モバイル*セグメントのTTFBは顕著に改善します。クリーンな有線経路では差は小さい。RUMデータをそのようにスライスすること。
...
| エッジレイテンシ p50 | < 50ms | `curl -w '%{time_starttransfer}'` またはRUM |
...
| 機能 | CloudFlare | Fastly | Akamai | AWS CloudFront |
|---------|------------|--------|--------|----------------|
| グローバルPoP | 285+ | 80+ | 4000+ | 450+ |
| エッジコンピュート | Workers | Compute@Edge | EdgeWorkers | Lambda@Edge |
| 即時パージ | 対応 | 対応 (<150ms) | 非対応 (~5s) | 対応 (~1min) |
| 無料枠 | 充実 | 制限付き | なし | 制限付き |
| WebSocket | 対応 | 対応 | 対応 | 対応 |
| リアルタイムログ | 対応 | 対応 | 対応 | 対応 |
