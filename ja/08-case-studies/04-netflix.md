...
Netflixは190以上の国で2億3,000万人以上の登録者にストリーミング配信しています。アーキテクチャの中心には、リアルタイムで品質を調整する**アダプティブビットレートストリーミング**（ABR）、世界中のISPに展開された**CDNインフラ**（Open Connect）、**マイクロサービスアーキテクチャ**（約1,000サービス）、コンテンツ発見の80%を推進する**レコメンデーションエンジン**、そして耐障害性を確保する**カオスエンジニアリング**があります。重要な知見：予測的キャッシュと分散配信によるバッファリングのない再生の最適化です。
...
### 機能要件
1. **動画ストリーミング** - アダプティブ品質でのコンテンツ再生
2. **コンテンツ発見** - パーソナライズされたレコメンデーションと検索
3. **ユーザープロフィール** - アカウントごとの複数プロフィールと設定
4. **ウォッチリスト管理** - 後で見るコンテンツの保存
5. **再生継続** - デバイス間での視聴再開
6. **コンテンツ取り込み** - 新規コンテンツのアップロードと処理

### 非機能要件
1. **可用性** - 99.99%のアップタイム（年間52分未満のダウンタイム）
2. **レイテンシ** - 動画開始 < 2秒、バッファリングなし
3. **スケール** - 2億3,000万人以上の登録者、1日4億時間以上のストリーミング
4. **グローバルリーチ** - 190以上の国で低レイテンシ配信
5. **耐障害性** - 障害時のグレースフルデグラデーション
...
| **登録者数** | 2億3,000万人以上 |
| **展開国数** | 190以上 |
| **ピーク同時ストリーミング数** | 1,500万以上 |
| **1日あたりのストリーミング時間** | 4億時間以上 |
| **帯域幅（ピーク時）** | グローバルインターネットトラフィックの15% |
| **CDNロケーション** | 17,000以上のOCAサーバー |
| **マイクロサービス数** | 約1,000 |
| **タイトルあたりのエンコード出力** | 1,200以上のファイル（全品質/コーデック組み合わせ） |
| **レコメンデーションAPIレイテンシ** | < 250ms p99 |
| **コンテンツライブラリ** | 15,000以上のタイトル |

---

## 主な学び

1. **コントロールプレーンとデータプレーンの分離** - APIロジックはAWSで実行し、動画配信は専用CDNで行います。それぞれ独立して最適化されます。

2. **タイトルごとのエンコーディング** - コンテンツの複雑さを分析してビットレートラダーを最適化します。シンプルなコンテンツは複雑なアクションシーケンスよりも少ないビットで済みます。

3. **プロアクティブなコンテンツ配置** - 視聴予測を使用して、オフピーク時間中にOCAを事前に充填します。ユーザーがリクエストする前にコンテンツが待機しています。

4. **クライアント側のインテリジェンス** - デバイス上のABRアルゴリズムが帯域幅とバッファ状態に基づいてリアルタイムの品質決定を行います。

5. **ISPとの深い統合** - Open ConnectアプライアンスをISPネットワークに直接埋め込むことで、トランジットコストを削減し品質を向上させます。

6. **レジリエンスを備えたマイクロサービス** - 1,000以上のサービスがサーキットブレーカー、リトライ、フォールバックを備えています。カオスエンジニアリングでレジリエンスを検証します。

7. **大規模なパーソナライゼーション** - すべてのリクエストでMLモデルが実行されます。EVCacheにより特徴量の検索レイテンシを削減します。

8. **マルチコーデック戦略** - H.264、H.265、VP9、AV1でエンコードし、デバイスの能力と帯域幅に最適化します。
