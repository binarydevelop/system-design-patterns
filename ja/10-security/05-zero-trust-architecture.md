...
> **Note:** This article was translated from English to Japanese. The original version is available at [`10-security/05-zero-trust-architecture.md`](../../10-security/05-zero-trust-architecture.md).
...
1. **クラウド導入**：リソースが複数のネットワークにまたがる
2. **リモートワーク**：ユーザーがどこからでも接続する
3. **BYOD**：個人デバイスが企業ネットワーク上にある
4. **サプライチェーン攻撃**：信頼されたベンダーが侵害される
5. **内部脅威**：悪意のあるまたは侵害された内部関係者
6. **高度な攻撃者**：境界はいずれ突破される
