...
**Layer 5 — Model quality.** Precision, recall, AUC, calibration, loss — computed once labels arrive and are joined back to the predictions that earned them. This is the only layer that tells the truth, and it is always the slowest. Everything above it exists to buy time against this layer's latency.
